...
def mqtt_main(
    broker_address, port, topic, queue, x_limit, stop_limit, dir_log, file_log
):
    ...
    global save_log
    if dir_log is None or file_log is None:
        save_log = None
    else:
    ...
    try:
        while True:
            time.sleep(1)
            if (
                float(time.time()) >= float(start_time) + float(x_limit) + float(5)
                and stop_limit
            ):
                break
